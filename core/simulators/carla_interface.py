...
from core.utils.simulator_utils.agents.tools.misc import is_within_distance_ahead
...
class CarlaInterface(object):
    ...
    def register_actor(self, actor: carla.Actor) -> None:
        ...
        if actor in self._actor_acceleration_map:
            raise KeyError("Vehicle '{}' already registered. Cannot register twice!".format(actor.id))
        else:
            self._actor_acceleration_map[actor] = None

        if actor in self._actor_angular_velocity_map:
            raise KeyError("Vehicle '{}' already registered. Cannot register twice!".format(actor.id))
        else:
            self._actor_angular_velocity_map[actor] = None

        if actor in self._actor_speed_vector_map:
            raise KeyError("Vehicle '{}' already registered. Cannot register twice!".format(actor.id))
        else:
            self._actor_speed_vector_map[actor] = None

    def register_actors(self, actors: List) -> None:
        """
    ...
    def on_carla_tick(self) -> None:
        """
        Callback from CARLA
        """
        for actor in self._actor_speed_map:
            if actor is not None and actor.is_alive:
                self._actor_speed_map[actor] = calculate_speed(actor)

        for actor in self._actor_transform_map:
            if actor is not None and actor.is_alive:
                self._actor_transform_map[actor] = actor.get_transform()

        for actor in self._actor_acceleration_map:
            if actor is not None and actor.is_alive:
                self._actor_acceleration_map[actor] = actor.get_acceleration()

        for actor in self._actor_angular_velocity_map:
            if actor is not None and actor.is_alive:
                self._actor_angular_velocity_map[actor] = actor.get_angular_velocity()

        for actor in self._actor_speed_vector_map:
            if actor is not None and actor.is_alive:
                self._actor_speed_vector_map[actor] = actor.get_velocity()
    ...
    def get_speed(self, actor: carla.Actor) -> float:
        ...
        """
        for key in self._actor_speed_map:
            if key.id == actor.id:
                return self._actor_speed_map[key]
        print('WARNING: {}.get_speed: {} not found!'.format(__name__, actor))
        return -1

    def get_velocity(self, actor: carla.Actor) -> float:
        """
        returns the absolute velocity for the given actor
        """
        return self.get_speed(actor)

    def get_transform(self, actor: carla.Actor) -> Optional[carla.Transform]:
        """
    ...
    def get_location(self, actor: carla.Actor) -> Optional[carla.Vector3D]:
        """
        returns the location for the given actor
        """
        for key in self._actor_transform_map:
            if key.id == actor.id:
                return self._actor_transform_map[key].location
        print('WARNING: {}.get_location: {} not found!'.format(__name__, actor))
        return None

    def get_acceleration(self, actor: carla.Actor) -> Optional[carla.Vector3D]:
        """
        returns the absolute acceleration for the given actor
        """
        for key in self._actor_acceleration_map:
            if key.id == actor.id:
                return self._actor_acceleration_map[key]
        print('WARNING: {}.get_acceleration: {} not found!'.format(__name__, actor))
        return None

    def get_angular_velocity(self, actor: carla.Actor) -> Optional[carla.Vector3D]:
        """
        returns the angular velocity for the given actor
        """
        for key in self._actor_angular_velocity_map:
            if key.id == actor.id:
                return self._actor_angular_velocity_map[key]
        print('WARNING: {}.get_angular_velocity: {} not found!'.format(__name__, actor))
        return None

    def get_speed_vector(self, actor: carla.Actor) -> Optional[carla.Vector3D]:
        """
        returns the absolute speed for the given actor
        """
    ...
    @client.setter
    def client(self, client: carla.Client) -> None:
        """
    ...
    @world.setter
    def world(self, world: carla.World) -> None:
        """
        Set the world and world settings
        """
        self._world = world
        self._sync_mode = world.get_settings().synchronous_mode
    ...
    @traffic_manager_port.setter
    def traffic_manager_port(self, tm_port: int) -> None:
        """
        Set the port to use for the traffic manager.
        """
        self._traffic_manager_port = tm_port

    def get_map(self, world: Optional[carla.World] = None) -> Any:
        """
        Get the current map
        """
    ...
    def get_ego_vehicle_route(self) -> List:
        ...
        """
        if self._hero_vehicle_route is not None:
    ...
    def create_blueprint(
        ...
        try:
            ...
            blueprint = np.random.choice(self._blueprint_library.filter(bp_filter))
    ...
    def request_new_actor(
        ...
        """
        blueprint = self.create_blueprint(model, rolename, color, actor_category, disable_two_wheels)

        if random_location:
            actor = None
            spawn_index = -1
            while not actor:
                spawn_index = self._rng.choice(list(self._available_points))
                spawn_point = self._spawn_points[spawn_index]
                actor = self._world.try_spawn_actor(blueprint, spawn_point)
            self._available_points.remove(spawn_index)
        ...
        if actor is None:
            ...
            if actor in self._blueprint_library.filter('vehicle.*'):
    ...
    def request_new_batch_actors(
        ...
        for i in range(amount):
            ...
            blueprint = self.create_blueprint(model, rolename, disable_two_wheels=disable_two_wheels)

            if random_location:
                if len(self._available_points) == 0:
    ...
    def handle_actor_batch(self, batch: List) -> Optional[List]:
        ...
        if self._client and batch is not None:
            responses = self._client.apply_batch_sync(batch, sync_mode)
        else:
            return None
        ...
        carla_actors = self._world.get_actors(actor_ids)
    ...
    def is_vehicle_hazard(self,
        ...
        for target_vehicle in vehicle_list:
            ...
            if is_within_distance_ahead(target_vehicle.get_transform(), self.get_transform(vehicle),
    ...
    def is_junction_vehicle_hazard(self, vehicle: carla.Actor,
        ...
        """
        vehicle_list = self.get_actor_list().filter("*vehicle*")
        o1 = _orientation(self.get_transform(vehicle).rotation.yaw)
        x1 = vehicle.bounding_box.extent.x
        p1 = self.get_location(vehicle) + self.get_transform(vehicle).get_forward_vector()
        ...
        if command == RoadOption.RIGHT:
            ...
            shift_angle = 0
        v1 = (5 * s1 + 6) * _orientation(self.get_transform(vehicle).rotation.yaw + shift_angle)

        for target_vehicle in vehicle_list:
            if target_vehicle.id == vehicle.id or not target_vehicle.is_alive:
                continue

            o2 = _orientation(self.get_transform(target_vehicle).rotation.yaw)
            o2_left = _orientation(self.get_transform(target_vehicle).rotation.yaw - 15)
            o2_right = _orientation(self.get_transform(target_vehicle).rotation.yaw + 15)
            x2 = target_vehicle.bounding_box.extent.x

            p2 = self.get_location(target_vehicle)
            p2_hat = p2 - (x2 + 2) * self.get_transform(target_vehicle).get_forward_vector()
    ...
    def is_bike_hazard(self, vehicle: carla.Actor) -> Tuple[bool, Optional[carla.Actor]]:
        ...
        for bike in bikes_list:
            if 'driver_id' not in bike.attributes or not bike.is_alive:
                continue
            o2 = _orientation(self.get_transform(bike).rotation.yaw)
    ...
    def is_walker_hazard(self, vehicle: carla.Actor) -> Tuple[bool, Optional[carla.Actor]]:
        ...
        p1 = _numpy(self.get_location(vehicle))
        v1 = 10.0 * _orientation(self.get_transform(vehicle).rotation.yaw)

        for walker in walkers_list:
            if not isinstance(walker, carla.Walker):
                continue
            v2_hat = _orientation(self.get_transform(walker).rotation.yaw)
    ...
    def clean_up(self) -> None:
        ...
        self._actor_speed_map.clear()
        self._actor_transform_map.clear()
        self._actor_acceleration_map.clear()
        self._traffic_light_map.clear()
        self._hero_vehicle_route = None
        self._target_waypoint = None
        self._map = None
    ...
    def remove_actors_in_surrounding(self, location, distance) -> None:
        """
        ...
        self._carla_actor_pool = dict({k: v for k, v in self._carla_actor_pool.items() if v})
