...
class CarlaEnvWrapper(gym.Wrapper):
    """
    Environment wrapper for Carla simulator to make ``gym.Env`` align with DI-engine definitions,
    so as to use utilities in DI-engine. It changes ``step``, ``reset`` and ``info`` method of ``gym.Env``,
    while others are straightly delivered.
...
class CarlaBenchmarkEnvWrapper(CarlaEnvWrapper):
    """
...
class CarlaScenarioEnvWrapper(CarlaEnvWrapper):
...
class MetaDriveEnvWrapper(gym.Wrapper):
    """
    Environment wrapper for all MetaDrive environments to make ``gym.Env`` align with DI-engine definitions.
