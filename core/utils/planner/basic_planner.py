...
class BasicPlanner(object):
    ...
    def __init__(self, cfg: Dict, carla_interface: 'CarlaInterface') -> None:
        ...
        self._world = self._carla_interface.world
    ...
    def run_step(self) -> None:
        ...
        vehicle_state, vehicle = self._carla_interface.is_vehicle_hazard(self._hero_vehicle)
        if not vehicle_state:
            vehicle_state, vehicle = self._carla_interface.is_lane_vehicle_hazard(
                self._hero_vehicle, self.target_road_option
            )
        if not vehicle_state:
            vehicle_state, vehicle = self._carla_interface.is_junction_vehicle_hazard(
                self._hero_vehicle, self.target_road_option
            )
        if vehicle_state:
            if self._debug:
        ...
        walker_state, walker = self._carla_interface.is_walker_hazard(self._hero_vehicle)
        if walker_state:
            if self._debug:
        ...
        light_state, traffic_light = self._carla_interface.is_light_red(self._hero_vehicle)
