...
class CarlaMVDetDataset(BaseIODataset):
    ...
    def __getitem__(self, idx):
        ...
        if np.max(np.abs(local_command_point)) > 500:
            _logger.info(str(self.measurements_files[idx]))
        local_command_point = R.T.dot(local_command_point)
        if any(np.isnan(local_command_point)):
            local_command_point[np.isnan(local_command_point)] = np.mean(
                local_command_point
            )
            _logger.info("NAN target point: %s" % self.measurements_files[idx])
